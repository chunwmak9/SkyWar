...
class Bullet(pygame.sprite.Sprite):
    ...
    def __init__(self,x,y):
        ...
        self.x = x
        self.y = y
        self.bullet_size = (60, 20)
        self.surf = pygame.image.load(DIR+"/Bullet.png")
        self.surf.set_colorkey((0,0,0),RLEACCEL)
        self.surf = pygame.transform.scale(self.surf, self.bullet_size )
...
from item_list import item_list as il

item_obj = il(screen,screen_width,screen_height)
occupied_pos = []
pos = []
...
while running:
    ...
    item_updates = item_obj.update()
    if item_updates!=None:
        screen.blit(item_updates, (0, 0))
    if item_obj.text_movement() != None:
        NFT_text = item_obj.text_movement()
        screen.blit(NFT_text, (200, item_obj.move_y))
        item_obj.move_y -=4

    if NFT_reward == 7:
        NFT = pygame.image.load(DIR+"/NFT_generator/Images/result.png") 
        item_obj.Append(NFT)
        print("Append")
        NFT_reward = 0
...
#####################################################################################################################################
                ...
                bullet = Bullet(player1.rect.left, player1.rect.top)
                bullet.bullet_size = (bullet.bullet_size[0]*5,bullet.bullet_size[1]*5)
                if bullet not in bullets:
                    bullets.add(bullet)
                    #all_sprites.add(bullet)
                    shoot_sound.set_volume(50)
    ...
    for bullet in bullets:
        ...
        for new_enemy in enemies:
            if pygame.sprite.spritecollideany(new_enemy, bullets):
                NFT_reward = random.randint(1,10)
    ...
    pygame.display.flip()
    clock.tick(60) #Maintain the speed of game at 40 frames per second
